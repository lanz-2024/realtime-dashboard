...
import { type ClientMessage, type ServerMessage, parseClientMessage } from './protocol';
...
function broadcast(channel: string, message: ServerMessage): void {
  ...
  for (const [sessionId, session] of sessions) {
    if (session.subscribedChannels.has(channel) || session.subscribedChannels.has('*')) {
  ...
  generationInterval = setInterval(() => {
    ...
    if (Math.random() < 0.02) {
      const alertMsg: ServerMessage = {
        type: 'alert',
        severity: Math.random() < 0.1 ? 'critical' : Math.random() < 0.3 ? 'warning' : 'info',
...
function generateAlertMessage(
  ...
  if (latencyValue > 500) return `P99 latency spike: ${latencyValue.toFixed(0)}ms`;
