 ...
 */
export function rollingAverage(points: TimeSeriesPoint[], windowSize: number): TimeSeriesPoint[] {
...
export function downsample(points: TimeSeriesPoint[], maxPoints: number): TimeSeriesPoint[] {
  ...
  for (let i = 0; i < maxPoints - 2; i++) {
    ...
    const bucketEnd = Math.min(Math.floor((i + 2) * bucketSize) + 1, points.length);
    ...
    const avgX = nextSlice.reduce((s, p) => s + p.timestamp, 0) / (nextSlice.length || 1);
    const avgY = nextSlice.reduce((s, p) => s + p.value, 0) / (nextSlice.length || 1);
